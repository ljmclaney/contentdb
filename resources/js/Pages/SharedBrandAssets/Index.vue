    ...
    <Layout>
        ...
        <div class="sticky top-0 z-40 bg-white rounded border-b border-gray-300 py-5 px-4 md:px-10 md:flex md:items-center md:justify-between mb-[30px]">
            <ul class="text-xl md:text-2xl font-bold flex items-center space-x-[10px]">
                <li class="truncate"><Link :href="route('viewSharedProject', [project.id, uuid])" class="text-black hover:text-indigo-500 transition-all">{{ project.name }}</Link></li>
...
export default {
    ...
    methods: {
        ...
        uploadFile() {
            ...
            this.form.post(route('sharedUploadFile', [this.project.id, this.uuid]), {
                preserveScroll: true,
                onSuccess: () => {
                    this.form.name = null
                    this.form.file = null
        ...
        deleteFile() {
            ...
            this.$inertia.delete(route('sharedDeleteFile', [this.project.id, this.uuid, this.selectedFileToDelete]))
        },
    }
}
</script>
